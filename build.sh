...
#
# Notes:
#   • No Apple cert here → the app is ad-hoc signed.
...
for a in "$@"; do
  case "$a" in
    ...
    *) echo "usage: ./build.sh [mac|win|all]"; exit 1 ;;
  esac
done
...
build_win() {
  npx electron-builder --win zip --x64 || echo "⚠ electron-builder win step failed (rcedit/wine) — falling back to manual zip"
...
case "$TARGET" in
  ...
  win) build_win ;;
  all) npx electron-builder --mac --universal && build_win ;;
esac
